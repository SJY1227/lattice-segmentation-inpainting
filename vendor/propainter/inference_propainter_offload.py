...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--save_fps', type=float, default=24, help='Frame per second. Default: 24')
    parser.add_argument(
    ...
    parser.add_argument(
        '--cpu_offload', action='store_true', help='Keep full-video tensors on CPU and process temporal windows on the device.')

    args = parser.parse_args()
    storage_device = torch.device('cpu') if args.cpu_offload else device
    ...
    frames, flow_masks, masks_dilated = frames.to(storage_device), flow_masks.to(storage_device), masks_dilated.to(storage_device)
    ...
    with torch.no_grad():
        ...
        if frames.size(1) > short_clip_len:
            ...
            for f in tqdm(range(0, video_length, short_clip_len), desc='RAFT flow'):
                ...
                flow_dtype = torch.float16 if args.cpu_offload and use_half else flows_f.dtype
                gt_flows_f_list.append(flows_f.to(storage_device, dtype=flow_dtype))
                gt_flows_b_list.append(flows_b.to(storage_device, dtype=flow_dtype))
                del flows_f, flows_b
            ...
            gt_flows_bi = tuple(flow.to(storage_device) for flow in fix_raft(frames.to(device), iters=args.raft_iter))
            torch.cuda.empty_cache()
        ...
        if flow_length > args.subvideo_length:
            ...
            pad_len = 5
            for f in tqdm(range(0, flow_length, args.subvideo_length), desc='Flow completion'):
                s_f = max(0, f - pad_len)
                e_f = min(flow_length, f + args.subvideo_length + pad_len)
                pad_len_s = max(0, f) - s_f
                pad_len_e = e_f - min(flow_length, f + args.subvideo_length)
                flow_chunk = tuple(flow[:, s_f:e_f].to(device) for flow in gt_flows_bi)
                flow_mask_chunk = flow_masks[:, s_f:e_f+1].to(device)
                pred_flows_bi_sub, _ = fix_flow_complete.forward_bidirect_flow(flow_chunk, flow_mask_chunk)
                pred_flows_bi_sub = fix_flow_complete.combine_flow(
                    flow_chunk, pred_flows_bi_sub, flow_mask_chunk)
                ...
                del flow_chunk, flow_mask_chunk, pred_flows_bi_sub
                torch.cuda.empty_cache()
            ...
            flow_chunk = tuple(flow.to(device) for flow in gt_flows_bi)
            flow_mask_chunk = flow_masks.to(device)
            pred_flows_bi, _ = fix_flow_complete.forward_bidirect_flow(flow_chunk, flow_mask_chunk)
            pred_flows_bi = fix_flow_complete.combine_flow(flow_chunk, pred_flows_bi, flow_mask_chunk)
            pred_flows_bi = tuple(flow.to(storage_device) for flow in pred_flows_bi)
            del flow_chunk, flow_mask_chunk
            torch.cuda.empty_cache()
        if args.cpu_offload:
        ...
        # ---- image propagation ----
        masked_frames = None if args.cpu_offload else frames * (1 - masks_dilated)
        subvideo_length_img_prop = min(100, args.subvideo_length) # ensure a minimum of 100 frames for image propagation
        if video_length > subvideo_length_img_prop:
            updated_frames, updated_masks = [], []
            pad_len = 10
            for f in tqdm(range(0, video_length, subvideo_length_img_prop), desc='Image propagation'):
                s_f = max(0, f - pad_len)
                ...
                pred_flows_bi_sub = tuple(flow[:, s_f:e_f-1].to(device) for flow in pred_flows_bi)
                frame_chunk = frames[:, s_f:e_f].to(device)
                mask_chunk = masks_dilated[:, s_f:e_f].to(device)
                masked_chunk = frame_chunk * (1 - mask_chunk) if args.cpu_offload else masked_frames[:, s_f:e_f]
                prop_imgs_sub, updated_local_masks_sub = model.img_propagation(
                    masked_chunk, pred_flows_bi_sub, mask_chunk, 'nearest')
                updated_frames_sub = frame_chunk * (1 - mask_chunk) + \
                                    prop_imgs_sub.view(b, t, 3, h, w) * mask_chunk
                ...
                del pred_flows_bi_sub, frame_chunk, mask_chunk, masked_chunk
                del prop_imgs_sub, updated_local_masks_sub, updated_frames_sub, updated_masks_sub
                torch.cuda.empty_cache()
            ...
            frame_chunk = frames.to(device)
            mask_chunk = masks_dilated.to(device)
            flow_chunk = tuple(flow.to(device) for flow in pred_flows_bi)
            masked_chunk = frame_chunk * (1 - mask_chunk) if args.cpu_offload else masked_frames
            prop_imgs, updated_local_masks = model.img_propagation(masked_chunk, flow_chunk, mask_chunk, 'nearest')
            updated_frames = (frame_chunk * (1 - mask_chunk) + prop_imgs.view(b, t, 3, h, w) * mask_chunk).to(storage_device)
            updated_masks = updated_local_masks.view(b, t, 1, h, w).to(storage_device)
            del frame_chunk, mask_chunk, flow_chunk, masked_chunk, prop_imgs, updated_local_masks
            torch.cuda.empty_cache()
        if args.cpu_offload:
            del frames, masked_frames
    ...
    for f in tqdm(range(0, video_length, neighbor_stride)):
        ...
        selected_pred_flows_bi = tuple(flow[:, neighbor_ids[:-1], :, :, :].to(device) for flow in pred_flows_bi)
